...
def get_data(data_path, ls_symbols):
    ...
    for symbol in ls_symbols:
        ...
        try:
            params= urllib.urlencode ({'output':'csv', 'q': str(symbol)})
            params = params + '&startdate=Jan+1%2C+2000'
            #we needn`t to set enddate, cause with empry param google return price till today
            print params
            url_get= urllib2.urlopen("http://www.google.com/finance/historical?%s" % params)
            print "http://www.google.com/finance/historical?%s" % params
